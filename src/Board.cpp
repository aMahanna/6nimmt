...
std::pair<std::vector<Card>, bool> Board::operator<<(const Card &c)
{
    ...
    if (index == -1)
    { // No valid row found, return false
        return make_pair(EMPTY_VECTOR, false);
    }
    else if (rows[index].size() != ROW_MAX)
    { // Valid row found, card inserted
        rows[index].push_back(c);
        return make_pair(EMPTY_VECTOR, true);
    }
    else
    { // Valid row found, but row is full
...
const std::string Board::toString() const
{
    ...
    {
        ss << "\nRow " << i + 1 << ": {";
        for (std::vector<Card>::const_iterator it = rows[i].begin(); it != rows[i].end(); it++)
        {
