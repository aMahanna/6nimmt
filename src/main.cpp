...
Card requestCard(Player &p) {
  ...
  do {
    std::cout << "--> " << p.getName()
              << ", choose a card from your hand to add onto the board: ";
    std::cin >> choice;
    ...
    if (std::cin.fail()) {
      ...
      for (std::vector<Card>::const_iterator it = p.begin(); it != p.end();
           it++) {
...
int main() {
  ...
  while (players.front().getPoints() > 0) {
    ...
    for (int i = 0; i < 10; i++) {
      for (Player &p : players) p += --gameDeck;
    }
    ...
    while (!players.front()) {
      ...
      for (auto cpPair : cardBank) {
        ...
        // of the operation
        std::pair<std::vector<Card>, bool> opResult =
            (gameBoard << cpPair.first);
        ...
        // pick up
        for (Card c : opResult.first) *(cpPair.second) -= *c;
      ...
      // Print the score
      players.sort();
      for (auto p : players) std::cout << p << std::endl;
    }
  }
